    ...
    pub fn apply_interpolated(
        ...
        match attr {
            ...
            StyleAttr::ShadowColor(color) => {
                if let Some(computed_shadow) = &computed.shadow {
                    _ = self.shadow.get_mut(entity).map(|mut shadow| {
                        shadow.0[0].color = lerp_color(&computed_shadow.0[0].color, color, ratio)
                    });
                }
            }
            StyleAttr::ShadowOffset(x, y) => {
                if let Some(computed_shadow) = &computed.shadow {
                    _ = self.shadow.get_mut(entity).map(|mut shadow| {
                        shadow.0[0].x_offset = lerp_val(&computed_shadow.0[0].x_offset, x, ratio);
                        shadow.0[0].y_offset = lerp_val(&computed_shadow.0[0].y_offset, y, ratio);
                    });
                }
            }
            StyleAttr::ShadowBlur(blur) => {
                if let Some(computed_shadow) = &computed.shadow {
                    _ = self.shadow.get_mut(entity).map(|mut shadow| {
                        shadow.0[0].blur_radius =
                            lerp_val(&computed_shadow.0[0].blur_radius, blur, ratio);
                    });
                }
            }
            StyleAttr::ShadowSpread(spread) => {
                if let Some(computed_shadow) = &computed.shadow {
                    _ = self.shadow.get_mut(entity).map(|mut shadow| {
                        shadow.0[0].spread_radius =
                            lerp_val(&computed_shadow.0[0].spread_radius, spread, ratio);
                    });
                }
            }
            _ => (),
    ...
    pub fn add_style_attr(&mut self, attr: StyleAttr) {
        match attr {
            ...
            StyleAttr::ShadowSpread(spread_radius) => match self.computed.shadow.as_mut() {
                Some(shadow) => shadow.0[0].spread_radius = spread_radius,
                None => {
                    self.computed.shadow = Some(BoxShadow(vec![ShadowStyle {
                        spread_radius,
                        ..default()
                    }]))
                }
            ...
            StyleAttr::ShadowBlur(blur_radius) => match self.computed.shadow.as_mut() {
                Some(shadow) => shadow.0[0].blur_radius = blur_radius,
                None => {
                    self.computed.shadow = Some(BoxShadow(vec![ShadowStyle {
                        blur_radius,
                        ..default()
            ...
            StyleAttr::ShadowColor(color) => match self.computed.shadow.as_mut() {
                Some(shadow) => shadow.0[0].color = color,
                None => {
                    self.computed.shadow =
                        Some(BoxShadow(vec![ShadowStyle { color, ..default() }]));
                }
            },
            StyleAttr::ShadowOffset(x, y) => match self.computed.shadow.as_mut() {
                Some(shadow) => {
                    shadow.0[0].x_offset = x;
                    shadow.0[0].y_offset = y;
                }
                None => {
                    self.computed.shadow = Some(BoxShadow(vec![ShadowStyle {
                        x_offset: x,
